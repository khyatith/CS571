...
class Onloadmobiledashboard extends CI_Controller
...
public function displaymobiledashboard()
{
...
$this->load->view('displaymobiledashboard',$data);
}
...
/*else if(!is_numeric($shippingzip) || $shippingzip==0)
{
$data['errormessage'] = 'Invalid Shipping Zip!';
echo $shippingzip;
$this->load->model('displaydashboard_model');
...
else if(!is_numeric($billingzip) || $billingzip==0)
{
$data['errormessage'] = 'Invalid Billing Zip!';
$this->load->model('displaydashboard_model');
$data['query']=$this->displaydashboard_model->display_dashboard($custusername);
...
{
$this->load->model('displaydashboard_model');
$this->displaydashboard_model->updateprofile($fname,$lname,$shippingstreet,$shippingcity,$shippingstate,$shippingzip,$billingstreet,$billingstate,$billingcity,$billingzip,$contact,$username,$email);
/*if($check==0)
{
...
public function displaymobileordersummary()
{
$this->load->model('displaydashboard_model');
$data['query1']=$this->displaydashboard_model->display_ordersummary();
$this->load->model('pc_model');
$data['query8'] = $this->pc_model->productcategory();
$this->load->view('displaymobileorder_summary',$data);
}
public function displaymobileorderdetails($ordernumber)
{
...
$this->load->model('pc_model');
$data['query8'] = $this->pc_model->productcategory();
$this->load->view('displaymobileorder_details',$data);
